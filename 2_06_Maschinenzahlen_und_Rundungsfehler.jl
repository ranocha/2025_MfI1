...
# ╔═╡ 8ed32d7b-0e64-419c-a454-e4abb5b03bcc
using BFloat16s
...
# ╔═╡ 48c7b960-2f22-4be0-a157-6182ec4a3a55
md"""
`Float64` ist für uns typischerweise der Standard-Zahlentyp, mit dem wir arbeiten. In C, C++ und MATLAB ist dies `double` (und der Standard wenn in MATLAB nichts anderes angegeben ist).
"""

# ╔═╡ 40fbd996-cc6d-4729-9b78-51c2a968ebc4
md"""
`Float32` wurde früher im wissenschaftlichen Rechnen verwendet, als die Hardware noch nicht so weit entwickelt war. Heutzutage wird `Float32` bspw. in der Computergraphik und auf GPUs genutzt.
"""

# ╔═╡ b13e1b1b-df36-482b-ab46-e42b632e183f
md"""
`Float16` wird im wissenschaftlichen Rechnen (noch) kaum verwendet. Solche Zahlentypen mit geringem Speicherbedarf sind allerdings für Anwendungen im maschinellen Lernen (künstliche Intelligenz, KI) sehr interessant.
"""

# ╔═╡ 041c8003-dff4-4bdf-b170-e73cdf27c3da
md"""
Von Google Brain wurde `BFloat16` als bessere Alternative zu `Float16` für KI-Anwendungen entwickelt.
"""
...
function plot_floating_point_precision(T)
	fig = Figure()
	fig[0, 1] = Label(fig, string(nameof(T)); tellwidth = false, font = :bold)
	
	xmin = nextfloat(zero(T))
	xmax = prevfloat(typemax(T))
	x = exp.(range(log(xmin), log(xmax), length = 1_000))
	x[begin] = xmin
	x[end] = xmax
	...
	ax_abs = Axis(fig[1, 1];
				  xlabel = "x", ylabel = "eps(x)",
			  	  xscale = log10, yscale = log10)
	lines!(ax_abs, x, e)
	xlims!(ax_abs, xmin, xmax)
	ylims!(ax_abs, extrema(e)...)
	
	ax_rel = Axis(fig[2, 1];
				  xlabel = "x", ylabel = "eps(x) / x",
			  	  xscale = log10, yscale = log10)
	lines!(ax_rel, x, e ./ x)
	lines!(ax_rel, [xmin, xmax], [eps(T), eps(T)];
		   label = "eps(" * string(nameof(T)) * ")")
	lines!(ax_rel, [floatmin(T), floatmin(T)], [eps(T), one(T)];
		   label = "floatmin(" * string(nameof(T)) * ")")
	axislegend(ax_rel)
	xlims!(ax_rel, xmin, xmax)

	linkxaxes!(ax_abs, ax_rel)
	
	fig
end

# ╔═╡ 4937124d-4ebc-4627-a26d-def94a4c754a
plot_floating_point_precision(Float64)

# ╔═╡ 08a273d7-b880-4c1e-b494-2576e68d4471
plot_floating_point_precision(Float32)

# ╔═╡ fe4f3928-9455-45be-ab22-255e4378aa82
plot_floating_point_precision(Float16)

# ╔═╡ f14c8cf0-ce59-4ab4-9987-dff7a5ecdf6e
plot_floating_point_precision(BFloat16)
...
manifest_format = "2.0"
project_hash = "1ba0468c6cd22000941c97e5ed29daad80fabeb6"
