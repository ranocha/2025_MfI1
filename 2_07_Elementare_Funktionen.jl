...
md"""
# 2.7 Elementare Funktionen

Die folgenden Abbildungen stellen einige elementare Funktionen dar:

- Polynom $x^2 - x$
- Potenz $x^3$
- Allgemeine Exponentialfunktion $3^x$
- Exponentialfunktion $\exp(x)$
- Kubikwurzel $\sqrt[3]{x}$
- Quadratwurzel $\sqrt{x}$
- Natürlicher Logarithmus $\log(x)$
- Logarithmus $\log_{10}(x)$

Welche der Abbildungen gehört zu welcher Funktion?
"""

# ╔═╡ 2ff1e076-a676-4286-8708-30a3ae617470
let
	fig = Figure(size = (1200, 450)) # default size is (600, 450)
	n = 500

	let ax = Axis(fig[1, 1]; xlabel = L"x", ylabel = L"y")
		x = range(-4, 2, length = n)
		lines!(ax, x, exp.(x))
	end

	let ax = Axis(fig[1, 2]; xlabel = L"x", ylabel = L"y")
		x = range(1.0e-2, 2.0, length = n)
		lines!(ax, x, log.(x))
	end

	let ax = Axis(fig[1, 3]; xlabel = L"x", ylabel = L"y")
		x = range(-2, 2, length = n)
		lines!(ax, x, @. x^3)
	end

	let ax = Axis(fig[1, 4]; xlabel = L"x", ylabel = L"y")
		x = range(-2, 2, length = n)
		lines!(ax, x, @. x^2 - x)
	end

	let ax = Axis(fig[2, 1]; xlabel = L"x", ylabel = L"y")
		x = range(-1, 2, length = n)
		lines!(ax, x, @. 3^x)
	end

	let ax = Axis(fig[2, 2]; xlabel = L"x", ylabel = L"y")
		x = range(1.0e-2, 2.0, length = n)
		lines!(ax, x, @. log(10, x))
	end

	let ax = Axis(fig[2, 3]; xlabel = L"x", ylabel = L"y")
		x = range(1.0e-2, 2.0, length = n)
		lines!(ax, x, cbrt.(x))
	end

	let ax = Axis(fig[2, 4]; xlabel = L"x", ylabel = L"y")
		x = range(1.0e-2, 2.0, length = n)
		lines!(ax, x, sqrt.(x))
	end
...
manifest_format = "2.0"
project_hash = "53de97edada8b5ba3428110450f0e94a024ae717"
